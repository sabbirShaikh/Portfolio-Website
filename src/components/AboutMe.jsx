    ...
    <section className="bg-gray-900 text-white py-12" id="about">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-10 lg:gap-16 mb-24 px-4 md:px-0">
          ...
          <div className="order-2 lg:order-1 text-left">
            <p className="text-gray-400 text-base md:text-lg lg:text-xl leading-relaxed mb-6">
              ...
              , a Software Developer with a passion for crafting dynamic
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="absolute left-4 md:left-8 top-0 h-full w-[2px] bg-gradient-to-b from-[#64ffda] via-gray-700 to-gray-800"></div>

          {courses.map((course, index) => {
            return (
              <div
                key={index}
                className="relative pl-12 md:pl-20 mb-12 group last:mb-0"
              >
                {/* Timeline Dot with Pulse Effect for Latest */}
                <div className="absolute left-4 md:left-8 -translate-x-1/2 top-7">
                  {index === 0 && (
                    <div className="absolute inset-0 w-4 h-4 rounded-full bg-[#64ffda]/40 animate-ping"></div>
                  )}
                  <div className="relative w-4 h-4 rounded-full bg-[#64ffda] border-4 border-gray-900 shadow-[0_0_12px_#64ffda] group-hover:scale-125 transition-transform duration-300"></div>
                </div>

                {/* Academic Card */}
                <div className="bg-gray-800/40 backdrop-blur-md border border-gray-700/60 rounded-2xl p-6 md:p-8 relative overflow-hidden transition-all duration-300 hover:border-[#64ffda]/40 hover:bg-gray-800/65 hover:shadow-[0_0_30px_rgba(100,255,218,0.06)]">
                  
                  {/* Faded Index Background Number */}
                  <span className="absolute top-2 right-6 text-7xl md:text-8xl font-black font-mono text-[#64ffda]/5 select-none pointer-events-none">
                    {String(courses.length - index).padStart(2, '0')}
                  </span>

                  {/* Year Tag */}
                  <div className="flex items-center">
                    <span className="inline-block px-3 py-1 text-xs font-mono font-semibold text-[#64ffda] bg-[#64ffda]/10 border border-[#64ffda]/20 rounded-full">
                      {course.year}
                    </span>
                  </div>

                  {/* Course Title */}
                  <h3 className="text-xl md:text-2xl font-bold text-white mt-3 group-hover:text-[#64ffda] transition-all duration-300">
                    {course.title}
                  </h3>

                  {/* Description */}
                  <p className="text-gray-400 text-sm md:text-base mt-2.5 leading-relaxed">
